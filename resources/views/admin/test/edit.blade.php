            ...
            <div class="col-12">
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">{{keywords()->Edit_Test ?? __('Edit Test')}}</h5>
                    </div>
                    <div class="card-body">
                        <form action="{{route('admin.test.update', $test)}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="row">
                                <div class="col-md-3">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" value="{{ $test->name }}" name="name" />
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="mb-3">
                                        <label class="form-label" for="employees-name">{{keywords()->Shortcut ?? __('Shortcut')}} </label>
                                        <input type="text" class="form-control" value="{{ $test->shortcut }}" name="shortcut" />
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="mb-3">
                                        <label class="form-label" for="employees-name">{{keywords()->Sample_Type ?? __('Sample Type')}} </label>
                                        <input type="text" class="form-control" value="{{ $test->sample_type }}" name="sample_type" />
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="mb-3">
                                        <label class="form-label" for="employees-name">{{keywords()->Price ?? __('Price')}} </label>
                                        <input type="text" class="form-control" value="{{ $test->price }}" name="price" />
                                    </div>
                                </div>
                                <div class="col-12">
                                    <div class="mb-3">
                                        <label class="form-label" for="employees-email">{{keywords()->Precautions ?? __('Precautions')}} </label>
                                        <textarea name="details" class="form-control" cols="30" rows="10">{{ $test->details }}</textarea>
                                    </div>
                                </div>
                            </div>
