    ...
    <div class="col-12">
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">{{keywords()->Create_Slider ?? __('Create Slider')}}</h5>
            </div>
            <div class="card-body">
                <form action="{{route('admin.slider.store')}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="employees-name" name="link" placeholder="{{keywords()->Enter_your_slider_link ?? __('Enter your slider link')}}..." />
                    </div>

                    <div class="mb-3">
                        <label class="form-label" for="employees-email">{{keywords()->Order ?? __('Order')}} </label>
                        <input type="number" class="form-control" id="employees-email" name="order" placeholder="{{keywords()->Auto ?? __('Auto')}}" />
                    </div>
