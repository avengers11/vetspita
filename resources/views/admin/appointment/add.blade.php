    ...
    <div class="col-12">
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">{{keywords()->Create_Appointment ?? __('Create Appointment')}}</h5>
            </div>
            <div class="card-body">
                <form action="{{route('admin.appointment.add')}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        ...
                        <select class="selectpicker" name="pet_id" id="select_client" data-live-search="true" required>
                            ...
                            @foreach ($patients as $item)
                                <option value="{{$item->id}}" data-subtext="{{ $item->unique_id }}">{{$item->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="selectpicker" name="doctor_id" id="select_client" data-live-search="true" required>
                            ...
                            @foreach ($doctors as $item)
                                <option value="{{$item->id}}" data-subtext="{{ $item->phone }}">{{$item->name}}</option>
                            @endforeach
                        </select>
                    </div>
