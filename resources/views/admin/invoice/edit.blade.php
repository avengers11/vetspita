    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <input type="text" style="background: #F3F6F9; border: 1px solid #F3F6F9; color: #999; cursor: pointer;" class="form-control mb-3" style="cursor: pointer" id="select-item-medicine" value="Medicine" readonly>

                <select class="selectpicker  mb-3" data-live-search="true" id="select-item-service">
                    <option value="">Select a service</option>
                    @foreach ($services as $item)
                        <option value="{{ $item->name }}" data-price="{{ $item->price }}">{{ $item->name }}</option>
                    @endforeach
                </select>
                <select class="selectpicker" data-live-search="true" id="select-item-test">
...
@push('js')
    <script>
        // Add Newe Item    
        $("#select-item-medicine").click(function(){
            addNewItemFileds("", "", "");
            $("#addItems .btn-close").click();
        });
        $("#select-item-service").change(function(){
            ...
            addNewItemFileds(service, price, "readonly");
            $("#addItems .btn-close").click();
        });
        $("#select-item-test").change(function(){
            let test = $(this).val();
            if(test == ""){
                return;
            }
            ...
            addNewItemFileds(test, price, "readonly");
            $("#addItems .btn-close").click();
        });
        const addNewItemFileds = (itemDetails, price, disabled) => {
            ...
            <tr>
                ...
                <td class="text-start">
                    <div class="mb-2">
                        <input type="text" name="item_details[]" class="form-control item-details" placeholder="Item Details" value="${itemDetails}" required />
                    </div>
                </td>
                <td>
