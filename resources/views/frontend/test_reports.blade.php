...
@section('content')
    ...
    <div>
        <div class="p-16 sm:p-28 bg-primary/30 flex flex-col items-center">
            <h1 class="text-5xl font-medium mb-7 text-center leading-snug">Test Reports</h1>
            ...
            <table class="w-full border">
                <thead>
                    <tr class="text-left px-4 py-3 border-b">
                        ...
                        <th class="px-4 py-3 text-sm text-gray-600 text-left border-r">REPORT ID</th>
                        ...
                        <th class="px-4 py-3 text-sm text-gray-600 text-left border-r">TEST NAME</th>
                ...
                <tbody>
                    @foreach ($tests as $index=>$test)
                        <tr class="text-left px-4 py-3 border-b">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-600 border-r">{{ $test->unique_id }}</td>
                            <td class="px-4 py-3 text-sm text-gray-600 border-r">{{ timeAgo($test->created_at) }}</td>
                            <td class="px-4 py-3 text-sm text-gray-600 border-r">{{ $test->pet_name }}</td>
                            <td class="px-4 py-3 text-sm text-gray-600 border-r">{{ $test->type }}</td>
                            <td class="px-4 py-3 text-sm text-gray-600 text-center">
                                <a href="{{ route("tests.view", $test) }}" type="button"
                    ...
                    @if(count($tests) < 1)
                        <tr class="text-left px-4 py-3 border-b">
                            <td class="px-4 py-3 text-sm text-gray-600 border-r text-center" colspan="6">No data found!</td>
                        </tr>
                    @endif
                </tbody>
            </table>
